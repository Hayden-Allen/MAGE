...
namespace n
{
	dynamic_list::dynamic_list() :
		mage::indexed_list<dynamic*, dynamic_list_handle, dynamic* const, dynamic* const>(c::dynamic_list_size, nullptr),
		m_vertex_buffer(new MAGE_RAN::dynamic_vertex_buffer(nullptr, c::dynamic_list_size & c::floats_per_dynamic)),
		m_vertex_array(new MAGE_RAN::dynamic_vertex_array(m_vertex_buffer,
			{
				// x, y
		...
		// each group contains up to MAGE_MTU dynamics
		m_batches(c::dynamic_list_size / MAGE_MTU)
	{}
	dynamic_list::~dynamic_list()
	{
	...
	{
		MAGE_ASSERT(is_valid(i), "Cannot update invalid dynamic {}", i);
