...
namespace n
{
	sprite_atlas::sprite_atlas(sprite_atlas_bank* const bank) :
		...
		m_texture(nullptr)
	{
		m_texture = mage::gfx::retained_texture2d::create(m_w, m_h, c::bytes_per_sprite_pixel * m_w * m_h, nullptr, s_texture_options);
	}
	sprite_atlas::sprite_atlas(mage::input_file& in) :
		dimensional<mage::gfx::retained_texture2d::s_type>(c::sprite_atlas_size, c::sprite_atlas_size),
		m_handle(0),
		m_texture(mage::gfx::retained_texture2d::create(m_w, m_h, c::bytes_per_sprite_pixel* m_w * m_h, s_texture_options))
	{
		load(in);
	}
