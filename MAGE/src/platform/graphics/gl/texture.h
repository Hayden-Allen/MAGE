...
#define T(NAME, TYPE, SIG) \
	...
	private: \
		void init(const texture_options& options) { glGenTextures(1, &m_id); bind(0); glTexParameteri(TYPE, GL_TEXTURE_MIN_FILTER, options.min_filter); glTexParameteri(TYPE, GL_TEXTURE_MAG_FILTER, options.mag_filter); glTexParameteri(TYPE, GL_TEXTURE_WRAP_S, options.wrap_s); glTexParameteri(TYPE, GL_TEXTURE_WRAP_T, options.wrap_t); } };
