...
namespace mage::gfx
{
	shader_program* shader_program::create(const std::initializer_list<shader*>& stages)
	{
		return new mage::gl::shader_program(stages);
	}
}
...
namespace mage::gl
{
	shader_program::shader_program(const std::initializer_list<mage::gfx::shader*>& stages) :
		mage::gfx::shader_program(stages)
	{
		m_id = glCreateProgram();

		bool vertex = false, fragment = false;
		for (const mage::gfx::shader* const s : stages)
		{
		...
		MAGE_CORE_ASSERT(vertex && fragment, "OpenGL shader program must have at least a vertex and fragment shader");
	...
	{
		set_uniform(glUniform1iv, name, MAGE_CAST(GLsizei, count), (const GLint*)b);
	}
	...
	{
		set_uniform(glUniform1iv, name, MAGE_CAST(GLsizei, count), i);
	}
	...
	{
		set_uniform(glUniform1uiv, name, MAGE_CAST(GLsizei, count), i);
	}
	...
	{
		MAGE_CORE_ERROR("OpenGL shader program {} error", s);
