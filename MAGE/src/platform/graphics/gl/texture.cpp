...
namespace mage::gfx
{
	...
	retained_texture2d* retained_texture2d::create(s_type w, s_type h, const uint32_t* const data, size_t count, const texture_options& options)
	{
		return new mage::gl::retained_texture2d(w, h, data, count, MAGE_PUN(mage::gl::texture_options, options));
	}
...
namespace mage::gl
{
	...
	retained_texture2d::retained_texture2d(s_type w, s_type h, const uint32_t* const data, size_t count, const texture_options& options) :
		mage::gfx::retained_texture2d(w, h, data, count),
		mage::dimensional<s_type>(w, h),
		m_options(options)
	{
		init(options);
